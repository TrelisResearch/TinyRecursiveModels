...
\begin{itemize}
    ...
    \item tama [40 tasks]: train + test example pairs
\end{itemize}
...
Post-training is done at competition time OR in a manner to simulate competition time training using a held out test set.

Four approaches to post-training were conducted:
1. Post-training from scratch. A null hypothesis that is no different than training from scratch, albeit with the limited level of competition-time compute available.
2. Full-fine tuning of a pretrained model.
3. Fine-tuning of embeddings only.
4. Full-fine tuning of a pretrained model, training only embeddings for the first quarter of optimizer steps.
5. LoRA fine-tuning of a pretrained model.

\subsubsection{Post-training from scratch.}
...
a) the model does learn that variants of the same task are related.
b) it would be more efficient to use the same task_id/embedding for all variants of a given task OR whether treating variants as independent tasks provides the model with some useful form of regularisation/generality during pre-training.

As a means of partially addressing a), one might look at the cosine of the angle between task embedddings for i) variants of the same task and ii) between the base/original/unvaried form of different tasks. If the TRM does indeed learn to encoded task variants similarly, perhaps one should expect a rise in the value cosine between embeddings during pre- and post-training. These cosine measures were charted during training of the second and third pre-trained models, and it was found that:
1. While the cosine does rise away from one, it remains low, suggesting that the model may not meaningfully recognise that variants are related.
2. The cosine of the angle between unvaried base tasks also rises but remains low. This is suggestive of there being little
...
This work was supported by Runpod, which provided \$15k of compute, and by Lambda Labs, which provided \$1k of compute. Thanks to Lewis Hemens for collaboration on ARC Prize research and Jack Boylan for assistance in running the pre-training replication.
